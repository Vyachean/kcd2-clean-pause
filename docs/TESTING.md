...
This branch is a **retail test candidate**, not a stable release. Installable test packages are generated by GitHub Actions from versioned repository source and published through GitHub Releases; generated ZIPs are not stored in Git.
...
Use the ZIP asset from the matching GitHub prerelease. The intended first candidate tag is:

```text
v0.1.0-rc.1
```

The tag workflow builds from the self-contained Xbox Store 1.5.6 release source committed with that tag and publishes both the install ZIP and `SHA256SUMS.txt`.
...
For maintainer/development reproduction only, the repository still supports:
...
Both builders fail closed if the profile does not match the expected Start routes/priority structure. Public test distribution goes through the tag-triggered `.github/workflows/release.yml` and `tools/build_release.py`.
