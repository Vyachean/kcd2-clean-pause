...
PATCHED_PROFILE_SOURCE = (
    ROOT / "vendor" / "kcd2" / "xbox-1.5.6" / "defaultProfile.clean-pause.xml.gz"
)
...
def read_release_profile() -> bytes:
    ...
    try:
        profile = gzip.decompress(PATCHED_PROFILE_SOURCE.read_bytes())
    except OSError as exc:
        raise ProfilePatchError(f"invalid release profile gzip: {exc}") from exc
    ...
    # Fail early if the vendored source is no longer valid UTF-8 XML text.
...
def build() -> Path:
    ...
    print(f"Release profile: {PATCHED_PROFILE_SOURCE}")
